...
function addBead() {
    ...
    $.each(beadData, function (e, val) {
        ...
        beadData.sr = i;
        var type = val.type;
        rows += `<tr id=${type.replace(/\s+/g, '') + finish_product_id + Math.floor(val.beads) + Math.floor(val.gram)}><td>${i}</td><td>${val.type}</td><td>${val.beads}</td><td style="text-align: right;">${val.gram}</td><td style="text-align: right;" >${val.carat}</td>
            ...
            <td><a class="text-danger text-white r${type.replace(/\s+/g, '') + finish_product_id + Math.floor(val.beads) + Math.floor(val.gram)}" onclick="Remove('${type.replace(/\s+/g, '') + finish_product_id + Math.floor(val.beads) + Math.floor(val.gram)}')"><i class="fa fa-trash"></i></a></td></tr>`;
...
function BeadByFinishDetail(finish_product_id) {
    $.ajax({
        type: "GET",
...
function Remove(id) {
    ...
    Swal.fire({
        ...
        $.each(beadData, function (i, val) {
            var type = val.type;
            if (type.replace(/\s+/g, '') + Math.floor(val.beads) + finish_product_id + Math.floor(val.gram) == id) {
